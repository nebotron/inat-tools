/**
 * Shared iNaturalist API v1 helpers.
 * Used by the observation explorer and the square-crop tool — same localStorage/cookie keys.
 */

export const INAT_API_V1 = "https://api.inaturalist.org/v1";

/** iNaturalist API v2 base (POST + `X-HTTP-Method-Override: GET` supports JSON bodies for observation search). */
export const INAT_API_V2 = "https://api.inaturalist.org/v2";
...
/** Value for the `Authorization` request header. Prefers OAuth bearer token; falls back to stored JWT. */
...
/**
 * POST to `/v2/...` with `X-HTTP-Method-Override: GET` so JSON (e.g. Elasticsearch `filters`) can be sent.
 * @param {string} relPath Path under v2, e.g. `observations` or `observations/species_counts`
 * @param {Record<string, unknown>} bodyObject JSON-serializable request body
 * @param {{ auth?: boolean, headers?: Record<string, string> }} [options]
 */
export function inatPostV2MethodOverrideGet(relPath, bodyObject, options = {}) {
  const path = String(relPath || "").replace(/^\//, "");
  const u = new URL(path, `${INAT_API_V2}/`);
  u.searchParams.set("_cb", `${Date.now()}-${Math.random().toString(36).slice(2, 11)}`);
  ...
  return fetch(u.href, {
    ...
    cache: "no-store",
    headers,
    body: JSON.stringify(bodyObject == null ? {} : bodyObject),
  });
}

// ─── OAuth helpers ────────────────────────────────────────────────────────────

const INAT_OAUTH_CLIENT_ID_COOKIE = "inatOAuthClientId";
const INAT_OAUTH_TOKEN_KEY = "inatOAuthToken";
const INAT_OAUTH_STATE_SESSION_KEY = "inatOAuthState";
const INAT_OAUTH_AUTHORIZE_URL = "https://www.inaturalist.org/dialog/authorize";
...
function deleteCookie(name) {
  document.cookie = `${encodeURIComponent(name)}=;expires=Thu, 01 Jan 1970 00:00:01 GMT;path=/;SameSite=Strict`;
}

/** Read the saved OAuth client ID from a cookie. */
export function getOAuthClientId() {
  return getCookie(INAT_OAUTH_CLIENT_ID_COOKIE);
}

/** Persist the OAuth client ID in a cookie (365-day expiry). Pass empty string to clear. */
export function setOAuthClientId(clientId) {
  const id = String(clientId || "").trim();
  if (id) setCookie(INAT_OAUTH_CLIENT_ID_COOKIE, id, 365);
  else deleteCookie(INAT_OAUTH_CLIENT_ID_COOKIE);
}

/** Read the stored OAuth access token from localStorage. */
export function getStoredOAuthToken() {
  try {
    return localStorage.getItem(INAT_OAUTH_TOKEN_KEY) || "";
  } catch {
    return "";
  }
}

/** Persist an OAuth access token in localStorage. */
export function storeOAuthToken(token) {
  try {
    if (token) localStorage.setItem(INAT_OAUTH_TOKEN_KEY, String(token));
    else localStorage.removeItem(INAT_OAUTH_TOKEN_KEY);
  } catch {
    /* ignore */
  }
}

/** Remove the stored OAuth access token. */
export function clearStoredOAuthToken() {
  try {
    localStorage.removeItem(INAT_OAUTH_TOKEN_KEY);
  } catch {
    /* ignore */
  }
}
...
/**
 * Redirect the browser to iNaturalist's OAuth authorization page (implicit grant flow).
 * Saves a random state value in sessionStorage for CSRF protection.
 * @param {string} clientId  The OAuth application's client ID.
 * @param {string} redirectUri  Must match the URI registered for the OAuth app.
 */
export function initiateOAuthFlow(clientId, redirectUri) {
  const state = (typeof crypto !== "undefined" && crypto.randomUUID)
    ? crypto.randomUUID()
    : Math.random().toString(36).slice(2) + Math.random().toString(36).slice(2);
  try {
    sessionStorage.setItem(INAT_OAUTH_STATE_SESSION_KEY, state);
  } catch {
    /* ignore */
  }
  const u = new URL(INAT_OAUTH_AUTHORIZE_URL);
...
/**
 * Call on page load to detect an OAuth redirect callback.
 * Reads `access_token` from the URL hash, validates state, stores the token,
 * and strips the hash from the URL. Returns the token string, or `null` if
...
export function consumeOAuthCallback() {
  ...
  let expectedState = "";
  try {
    expectedState = sessionStorage.getItem(INAT_OAUTH_STATE_SESSION_KEY) || "";
    sessionStorage.removeItem(INAT_OAUTH_STATE_SESSION_KEY);
  } catch {
    /* ignore */
  }
